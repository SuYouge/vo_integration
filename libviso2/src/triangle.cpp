...
void printsubseg(struct mesh *m, struct osub *s)
{
  struct osub printsh;
  struct otri printtri;
  vertex printvertex;
...
float incircleadapt(vertex pa, vertex pb, vertex pc, vertex pd, float permanent)
{
  ...
  int cxtablen(0); //
  int cytablen(0); // 
  // modified for warniing about uninitialized variables, using direct init
...
void vertexsort(vertex *sortarray, int arraysize)
{
  ...
  if ((arraysize >= 2 )&& (right < arraysize - 2)) {
                         ...
                         vertex endpoint2)
{
  ...
  success = insertvertex(m, b, newvertex, splittri, splitsubseg, 0);
